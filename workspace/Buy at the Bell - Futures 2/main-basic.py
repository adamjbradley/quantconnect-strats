...
class BubbleAlgorithm(QCAlgorithm):
    ...
    def initialize(self):
        ...
        for stock in self._symbols:
            self.add_security(SecurityType.EQUITY, stock, Resolution.DAILY)
            
            self._macd = self.macd(stock, 12, 26, 9, MovingAverageType.EXPONENTIAL, Resolution.DAILY)
            self._macd_dic[stock] = self._macd


            self._bb = self.bb(stock, 30, 2, MovingAverageType.Simple, Resolution.DAILY)
            self._bb_dic[stock] = self._bb

            self._rsi = self.rsi(stock, 14, MovingAverageType.EXPONENTIAL, Resolution.DAILY)
            self._rsi_dic[stock] = self._rsi

            self._atr = self.atr(stock, 1)
            self._atr_dic[stock] = self._atr

            self._bbw = (self._bb.Current.Value - self._bb.LowerBand.Current.Value) / self._bb.MiddleBand.Current.Value
